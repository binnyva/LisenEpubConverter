...
import { defaultAudioSettings, defaultLlmSettings, loadPreferredAudioModels, loadPreferredModels, preferredAudioModelsFile, preferredModelsFile } from './models.js';
import { reconcileUnresolvedSpeakers } from '../pipeline/reconcile-speakers.js';
  ...
  const server = http.createServer(async (request, response) => {
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/api/settings') {
        return json(response, 200, {
          ...defaultLlmSettings(),
          models: loadPreferredModels(),
          modelsFile: preferredModelsFile,
          audio: {
            ...defaultAudioSettings(),
            models: loadPreferredAudioModels(),
            modelsFile: preferredAudioModelsFile,
          },
        });
      }
      ...
      if (request.method === 'POST' && url.pathname === '/api/run') {
        ...
        const body = await bodyJson<{ epub?: string; stage?: Stage; chapters?: number[]; rerun?: boolean; rebuild?: boolean; llm?: { provider?: string; model?: string }; audio?: { provider?: string; model?: string } }>(request);
        ...
        const audio = parseAudioSettings(body.audio);
        const job: LocalJob = {
      ...
      if (request.method === 'POST' && url.pathname === '/api/speaker-reconciliation') {
        if (activeJob) return json(response, 409, { error: `A ${activeJob.stage} job is already running.` });
        const body = await bodyJson<{ epub?: string; llm?: { provider?: string; model?: string } }>(request);
        const epubPath = requireEpub(body.epub);
        const work = new WorkDir(epubPath, workRoot);
        if (work.sourceChanged()) throw new Error('The selected EPUB differs from the source used for this work folder. Rebuild Extract before reconciling speakers.');
        const llm = parseLlmSettings(body.llm);
        const suggestions = await withLlmRunConfig(llm, () => reconcileUnresolvedSpeakers(work));
...
function parseAudioSettings(value?: { provider?: string; model?: string }): { provider: ProviderId; model: string } {
  const fallback = defaultAudioSettings();
  const provider = value?.provider ?? fallback.provider;
  if (provider !== 'openai' && provider !== 'openrouter') throw new Error('Audio provider must be "openai" or "openrouter".');
  const model = value?.model?.trim() ?? fallback.model;
  if (!model) throw new Error('Enter an audio model name.');
  if (model.length > 200) throw new Error('Audio model names must be 200 characters or fewer.');
  return { provider, model };
}
...
</style></head><body><main class="shell"><header><div><p class="eyebrow">Local audiobook production</p><h1>Lisen workspace</h1></div><div class="header-actions"><p class="subtle">Files and API keys stay on this machine.</p><button class="quiet icon" id="open-settings" aria-label="Model settings" title="Model settings">&#9881;</button></div></header><section class="panel"><form class="open" id="open-form"><input id="epub" placeholder="/full/path/to/book.epub" aria-label="EPUB path" required><button>Open EPUB</button></form><div id="books" class="books"></div></section><section id="activity" class="panel activity" hidden aria-labelledby="activity-title"><h2 id="activity-title">Stage activity</h2><p id="activity-status" class="subtle" role="status"></p><div id="chapter-progress" class="chapter-progress" hidden><h3 id="progress-title"></h3><p id="progress-phase" class="subtle" role="status"></p><progress id="progress-bar" max="100" value="0" aria-label="Current activity progress"></progress><p id="progress-text" class="subtle"></p><p id="progress-chapters" class="subtle"></p></div><div id="activity-log" class="activity-log" role="log" aria-live="polite" aria-relevant="additions"></div></section><section id="workspace" class="workspace" hidden><div class="panel"><h2 id="book-title">Pipeline</h2><div id="stages" class="stages"></div><div id="chapter-area" hidden><div class="chapter-tools"><button class="quiet" id="select-all">Select narratable</button><button class="quiet" id="clear-selection">Clear</button><span class="subtle" id="selected-count"></span></div><div id="chapters" class="chapter-list"></div></div></div><aside class="detail"><div class="panel"><h2>Book analysis</h2><p class="subtle">Sampled overview; character candidates are provisional.</p><pre class="json" id="analysis"></pre></div><div class="panel"><h2>Characters</h2><p class="subtle" id="character-status"></p><div id="characters"></div></div><div class="panel"><h2>Voice casting</h2><p class="subtle">Saving changes makes synthesis and assembly stale.</p><form class="cast" id="casting"></form><button id="save-casting">Save casting</button></div></aside></section></main><dialog id="settings-dialog" aria-labelledby="settings-title"><h2 id="settings-title">Model settings</h2><p class="subtle">Text models are used for Analyze, Chapters, List Characters, Script, and Casting. The selected audio model is recorded when you next run Voices; Synthesis uses that saved voice binding.</p><div class="settings-grid"><section><h3>Text processing</h3><label>Provider<select id="llm-provider" aria-label="Text provider"><option value="openai">OpenAI</option><option value="openrouter">OpenRouter</option></select></label><label>Model<div class="model-picker"><input id="llm-model" autocomplete="off" spellcheck="false" aria-autocomplete="list" aria-controls="model-suggestions" placeholder="Enter a model name"><div id="model-suggestions" class="suggestions" role="listbox"></div></div></label><div><h3>Starred text models</h3><div id="starred-models" class="starred-models"></div></div><p class="subtle settings-file">Edit <code id="models-file"></code> to add text models or set <code>starred</code> to true.</p></section><section><h3>Audio synthesis</h3><label>Provider<select id="audio-provider" aria-label="Audio provider"><option value="openai">OpenAI</option><option value="openrouter">OpenRouter</option></select></label><label>Model<div class="model-picker"><input id="audio-model" autocomplete="off" spellcheck="false" aria-autocomplete="list" aria-controls="audio-model-suggestions" placeholder="Enter an audio model name"><div id="audio-model-suggestions" class="suggestions" role="listbox"></div></div></label><div><h3>Starred audio models</h3><div id="starred-audio-models" class="starred-models"></div></div><p class="subtle settings-file">Edit <code id="audio-models-file"></code> to add audio models or set <code>starred</code> to true.</p></section></div><div class="dialog-actions"><button class="quiet" id="close-settings">Close</button><button id="apply-settings">Use for new runs</button></div></dialog><dialog id="cmd-dialog" aria-labelledby="cmd-title"><h2 id="cmd-title">Run from the command line</h2><pre id="cmd-text" class="command"></pre><div class="dialog-actions"><button class="quiet" id="close-cmd">Close</button><button id="copy-cmd">Copy command</button></div></dialog><div id="toast" class="toast" role="status" aria-live="polite"></div><script>
if(document.body)document.body.insertAdjacentHTML('beforeend','<style>.text-link{display:inline-block;margin-top:10px;background:none;border:0;padding:0;color:#9ff1d2;font-weight:700;text-decoration:underline;cursor:pointer}.character-dialog{width:min(900px,calc(100vw - 32px));max-height:calc(100vh - 32px);overflow:auto}.character-review-tools{margin:0 0 12px}.character-editors{display:grid;gap:14px}.character-editor{padding:15px;border:1px solid #3c5177;border-radius:12px;background:#101a30}.character-editor.needs-review{border-color:#a88448}.character-editor h3{margin:0 0 10px}.character-editor label{display:grid;gap:5px;font-size:.78rem;color:#bcd1f6}.character-fields{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.character-fields .wide{grid-column:span 2}.character-issues{margin:0 0 10px;padding-left:20px;color:#f3bd63;font-size:.85rem}.character-evidence{margin:12px 0 0}.character-evidence p{margin:7px 0;font-size:.82rem;white-space:pre-wrap}@media(max-width:640px){.character-fields{grid-template-columns:1fr}.character-fields .wide{grid-column:span 1}}</style><dialog id="character-dialog" class="character-dialog" aria-labelledby="character-dialog-title"><h2 id="character-dialog-title">Review book characters</h2><p class="subtle">Correct registry details before scripting. Saving makes Script and later stages stale, but keeps the audio cache.</p><div class="character-review-tools"><button class="quiet" type="button" id="show-blocking-character-issues" aria-pressed="false">Show blocking issues only</button></div><p id="character-review-summary" class="subtle"></p><div id="character-editors" class="character-editors"></div><div class="dialog-actions"><button class="quiet" id="ask-llm-resolutions">Ask LLM for proposals</button><button class="quiet" id="close-character-dialog">Cancel</button><button id="save-characters">Save character registry</button></div></dialog>');
...
const stages=['extract','analyze','chapters','list-characters','script','casting','voices','synth','assemble'];let book=null,voices=[],selected=new Set(),job=null,toastTimer=null,activityJobId=null,activityEventCount=0,activityError=null,modelSettings=null,activeLlm=null,activeAudio=null,candidateResolutions={},candidateSuggestions={},scriptResumeFrom=null,showBlockingCharacterIssues=false;
...
function selectedAudio(){return activeAudio||(modelSettings?.audio?{provider:modelSettings.audio.provider,model:modelSettings.audio.model}:null)}
function renderModelSettings(){if(!modelSettings)return;const current=selectedLlm(),audio=selectedAudio(),provider=$('#llm-provider'),input=$('#llm-model'),audioProvider=$('#audio-provider'),audioInput=$('#audio-model');provider.value=current.provider;input.value=current.model;audioProvider.value=audio.provider;audioInput.value=audio.model;$('#models-file').textContent=modelSettings.modelsFile;$('#audio-models-file').textContent=modelSettings.audio.modelsFile;renderModelChoices();renderAudioModelChoices()}
...
function renderAudioModelChoices(){if(!modelSettings?.audio)return;const provider=$('#audio-provider').value,query=$('#audio-model').value;const matches=modelSettings.audio.models.filter(m=>m.provider===provider).map(m=>({...m,score:fuzzyScore(m.model,query)})).filter(m=>m.score>=0).sort((a,b)=>a.score-b.score||a.model.localeCompare(b.model));$('#audio-model-suggestions').innerHTML=matches.slice(0,8).map(m=>'<button class="suggestion" role="option" data-audio-model-choice="'+esc(m.model)+'">'+esc(m.model)+'</button>').join('');document.querySelectorAll('[data-audio-model-choice]').forEach(x=>x.onclick=()=>{ $('#audio-model').value=x.dataset.audioModelChoice;renderAudioModelChoices();$('#audio-model').focus() });const starred=modelSettings.audio.models.filter(m=>m.provider===provider&&m.starred);$('#starred-audio-models').innerHTML=starred.length?starred.map(m=>'<button class="quiet" data-audio-model-choice="'+esc(m.model)+'">&#9733; '+esc(m.model)+'</button>').join(''):'<p class="subtle">No starred '+esc(provider)+' audio models yet.</p>';document.querySelectorAll('#starred-audio-models [data-audio-model-choice]').forEach(x=>x.onclick=()=>{ $('#audio-model').value=x.dataset.audioModelChoice;renderAudioModelChoices() })}
...
function updateCharacterReviewFilter(){const button=$('#show-blocking-character-issues');button.textContent=showBlockingCharacterIssues?'Show all character issues':'Show blocking issues only';button.setAttribute('aria-pressed',String(showBlockingCharacterIssues));document.querySelectorAll('[data-character-editor]').forEach(row=>{row.hidden=showBlockingCharacterIssues&&!row.classList.contains('blocking')});document.querySelectorAll('[data-candidate-editor]').forEach(row=>{row.hidden=showBlockingCharacterIssues&&Boolean(row.querySelector('[data-candidate-resolution]').value)})}
function refreshCharacterReviewIssues(){const issues=reviewIssues(dialogCharacters());let flagged=0,blocking=0;document.querySelectorAll('[data-character-editor]').forEach(row=>{const list=issues.get(row.dataset.id)||[],hasBlocking=list.some(isBlockingIssue),visibleIssues=showBlockingCharacterIssues?list.filter(isBlockingIssue):list;flagged+=list.length?1:0;blocking+=hasBlocking?1:0;row.classList.toggle('needs-review',list.length>0);row.classList.toggle('blocking',hasBlocking);row.querySelector('[data-character-issues]').innerHTML=visibleIssues.map(issueMarkup).join('')||'<li class="subtle">No '+(showBlockingCharacterIssues?'blocking ':'')+'review issues found.</li>';row.querySelectorAll('[data-conflict-choice]').forEach(choice=>choice.onclick=()=>{row.querySelector('[data-character-field="'+choice.dataset.conflictField+'"]').value=choice.dataset.conflictValue;refreshCharacterReviewIssues()});const status=row.querySelector('[data-character-status]');status.textContent=hasBlocking?'Fix before saving':list.length?'Needs review':'Ready';status.className='character-status '+(hasBlocking?'blocking':list.length?'review':'')});const unresolved=unresolvedCandidateCount(),totalBlocking=blocking+unresolved;$('#character-review-summary').textContent=totalBlocking?totalBlocking+' blocking '+(totalBlocking===1?'issue remains':'issues remain')+' before Script can run. '+(unresolved?unresolved+' unresolved speaker '+(unresolved===1?'needs':'need')+' a resolution. ':'')+(flagged-blocking?String(flagged-blocking)+' character '+(flagged-blocking===1?'needs':'need')+' review.':''):flagged?flagged+' character '+(flagged===1?'needs':'need')+' review.':'No duplicate, unresolved, or incomplete profiles found.';updateCharacterReviewFilter()}
...
function characterResolutionLabel(character){const chapterIndex=character.chapters?.[0]??character.evidence?.[0]?.chapter;const chapter=book?.chapters?.find(item=>item.index===chapterIndex);const chapterNumber=Number.isInteger(chapterIndex)?chapterIndex+1:'?';const name=character.name||character.aliases?.[0]||'Unnamed character';const title=chapter?.title?.trim();return 'C '+chapterNumber+': '+name+(title?" (from Chapter '"+title+"')":'')}
function candidateMarkup(candidate,registry){const resolution=candidateResolutions[candidate.key]||'',selected=resolution==='new'?'new':resolution,suggestion=candidateSuggestions[candidate.key];return '<section class="unresolved-candidate" data-candidate-editor data-key="'+esc(candidate.key)+'"><h3>Unresolved speaker: '+esc(candidate.speaker)+' <span class="subtle">· chapters '+candidate.chapters.map(index=>index+1).join(', ')+'</span></h3><p class="subtle" data-candidate-status>This speaker blocks Script until it is matched or added.</p>'+(suggestion?'<p class="subtle"><strong>LLM proposal:</strong> '+esc(suggestion.reason)+(suggestion.evidence?' Evidence: '+esc(suggestion.evidence):'')+'</p>':'')+'<div class="candidate-resolution"><label>Resolve as<select data-candidate-resolution><option value="">Choose a resolution…</option><option value="new" '+(selected==='new'?'selected':'')+'>Add '+esc(candidate.speaker)+' as a new character</option>'+registry.characters.map(character=>'<option value="'+esc(character.id)+'" '+(selected===character.id?'selected':'')+'>'+esc(characterResolutionLabel(character))+'</option>').join('')+'</select></label></div><details class="candidate-samples"><summary>Script evidence · '+candidate.samples.length+' sample'+(candidate.samples.length===1?'':'s')+'</summary>'+candidate.samples.map(sample=>'<p><strong>Chapter '+(sample.chapter+1)+':</strong> '+esc(sample.text)+(sample.delivery?' <span class="subtle">· '+esc(sample.delivery)+'</span>':'')+'</p>').join('')+'</details></section>'}
...
function renderCharacterDialog(){const registry=book.characterRegistry;if(!registry)return;const candidates=(book.characterCandidates||[]).map(candidate=>candidateMarkup(candidate,registry)).join('');$('#ask-llm-resolutions').hidden=!candidates;$('#character-editors').innerHTML=candidates+registry.characters.map(character=>'<details class="character-editor" data-character-editor data-id="'+esc(character.id)+'"><summary><span>'+esc(character.name||'Unnamed character')+' <span class="subtle">· chapters '+character.chapters.map(index=>index+1).join(', ')+'</span></span><span class="character-status" data-character-status></span></summary><div class="character-body"><ul class="character-issues" data-character-issues></ul><div class="character-fields">'+field('Name','name',character.name)+field('Aliases (comma-separated)','aliases',character.aliases.join(', '),'wide')+selectField('Sex','sex',character.sex,['unknown','female','male'])+field('Age','age',character.age)+field('Race','race',character.race)+field('Role / class','class',character.class)+field('Country / accent','country',character.country)+selectField('Importance','importance',character.importance,['main','secondary','minor'])+'</div><details class="character-evidence"><summary>Evidence · '+character.evidence.length+' item'+(character.evidence.length===1?'':'s')+'</summary>'+character.evidence.map(item=>'<p><strong>Chapter '+(item.chapter+1)+', passage '+(item.chunk+1)+':</strong> '+esc(item.text)+'</p>').join('')+'</details></div></details>').join('');document.querySelectorAll('[data-character-editor] input,[data-character-editor] select').forEach(input=>{input.oninput=refreshCharacterReviewIssues;input.onchange=refreshCharacterReviewIssues});document.querySelectorAll('[data-character-editor]').forEach(row=>row.ontoggle=()=>{if(row.open)document.querySelectorAll('[data-character-editor]').forEach(other=>{if(other!==row)other.open=false})});document.querySelectorAll('[data-candidate-resolution]').forEach(choice=>choice.onchange=refreshCandidateResolutions);refreshCandidateResolutions()}
function openCharacterDialog(event){if(event)event.preventDefault();if(!book.characterRegistry)return;candidateResolutions={};candidateSuggestions={};showBlockingCharacterIssues=false;renderCharacterDialog();$('#character-dialog').showModal()}
function applySpeakerSuggestions(suggestions){candidateResolutions={};candidateSuggestions={};for(const suggestion of suggestions||[]){candidateSuggestions[suggestion.key]=suggestion;if(suggestion.resolution==='existing'&&suggestion.characterId)candidateResolutions[suggestion.key]=suggestion.characterId;else if(suggestion.resolution==='new')candidateResolutions[suggestion.key]='new'}}
async function askLlmForSpeakerResolutions(){if(!book?.epubPath)return;try{notice('Asking the LLM to reconcile unresolved speakers…','progress');const result=await api('/api/speaker-reconciliation',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({epub:book.epubPath,llm:selectedLlm()})});book=await api('/api/book?epub='+encodeURIComponent(book.epubPath));applySpeakerSuggestions(result.suggestions);render();renderCharacterDialog();$('#character-dialog').showModal();const proposed=(result.suggestions||[]).filter(s=>s.resolution!=='unresolved').length;notice(proposed?'Review '+proposed+' LLM proposal'+(proposed===1?'':'s')+' and save to resume Script.':'The LLM could not support a resolution; review the evidence manually.','success')}catch(e){notice(e.message,'error')}}
...
function renderJob(current){
  ...
  if(current.status==='failed'&&current.stage==='script'&&/New or unresolved speakers/.test(current.error||'')&&!log.querySelector('[data-reconcile-speakers]')){const action=document.createElement('button');action.className='quiet';action.textContent='Ask LLM to propose speaker resolutions';action.dataset.reconcileSpeakers='';action.onclick=askLlmForSpeakerResolutions;log.append(action)}
