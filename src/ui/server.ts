...
import { CastingSchema, type Analysis, type BookMetadata, type Casting } from '../types.js';
  ...
  const server = http.createServer(async (request, response) => {
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/api/voices') {
        return json(response, 200, getTTSProvider().listVoices());
      }
      ...
      if (request.method === 'POST' && url.pathname === '/api/casting') {
        const body = await bodyJson<{ epub?: string; casting?: Casting }>(request);
        const epubPath = requireEpub(body.epub);
...
function bookDetailFromRoot(root: string, workRoot: string, stateOverride?: WorkState): object {
  ...
  return { epubPath, epubAvailable: fs.existsSync(epubPath), root: rootResolved, state, metadata, analysis, casting, chapters };
}
...
const stages=['extract','analyze','chapters','script','casting','synth','assemble'];let book=null,voices=[],selected=new Set(),job=null,toastTimer=null;
...
function stageHint(stage,state){const descriptions={extract:'Read the EPUB into chapters, metadata, and cover art.',analyze:'Identify characters, summarize the book, and choose what to narrate.',chapters:'Clean selected chapters for narration and create chapter summaries.',script:'Split prose and dialogue into speaker-attributed segments.',casting:'Choose a voice and delivery instructions for each speaker.',synth:'Turn selected script segments into cached speech audio. TTS usage is billable.',assemble:'Combine synthesized chapters into an M4B with chapter markers.'};if(state==='done')return 'Complete · '+descriptions[stage];if(state==='running')return 'Running locally…';return state==='stale'?'Needs a fresh run · '+descriptions[stage]:descriptions[stage]}
...
$('#open-form').onsubmit=e=>{e.preventDefault();openBook($('#epub').value)};$('#select-all').onclick=()=>{book.chapters.filter(c=>c.narrate).forEach(c=>selected.add(c.index));render()};$('#clear-selection').onclick=()=>{selected.clear();render()};$('#close-cmd').onclick=()=>$('#cmd-dialog').close();$('#copy-cmd').onclick=copyCommand;$('#save-casting').onclick=async()=>{if(!book.casting)return;const next={narrator:{...book.casting.narrator},characters:{}};document.querySelectorAll('[data-voice]').forEach(x=>{const name=x.dataset.voice;const instructions=document.querySelector('[data-instructions="'+CSS.escape(name)+'"]').value;const item={voiceId:x.value,instructions};if(name==='narrator')next.narrator=item;else next.characters[name]=item});try{book=await api('/api/casting',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({epub:book.epubPath,casting:next})});notice('Casting saved. Existing audio is now stale.','success');render();refreshBooks()}catch(e){notice(e.message,'error')}};Promise.all([refreshBooks(),api('/api/voices')]).then(([,v])=>{voices=v}).catch(e=>notice(e.message,'error'));
</script></body></html>`;
